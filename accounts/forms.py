...
class UserCreationForm(UserCreationForm):
    ...
    class Meta:
        model = User
        fields = ('username', 'name', 'email', 'institution') 
        widgets = {
            'username': forms.TextInput(attrs={'class': 'form-control'}),
...
class Autenticacao(AuthenticationForm):
    username = forms.CharField(max_length=254, label="Usuário", widget=forms.TextInput(attrs={'class': 'form-control form-control-sm'}))
    password = forms.CharField(label="Senha", widget=forms.PasswordInput(attrs={'class': 'form-control form-control-sm'}))
